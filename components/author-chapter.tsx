...
function BrandSeal({ id }: { id: string }) {
  ...
  return <svg className={styles.seal} viewBox="0 0 120 120" aria-hidden="true" focusable="false">
    <defs><path id={pathId} d="M 15,60 a 45,45 0 1,1 90,0 a 45,45 0 1,1 -90,0" /></defs>
    <text className={styles.sealText} textLength="280" lengthAdjust="spacing"><textPath href={`#${pathId}`} startOffset="1%">ESTÉTICA • EQUILÍBRIO • LAÍS PAULA • </textPath></text>
    <text className={styles.sealMark} x="60" y="70" textAnchor="middle">lp.</text>
    <circle cx="8" cy="60" r="1.6" /><circle cx="112" cy="60" r="1.6" />
  </svg>;
}
...
export function AuthorChapter() {
  ...
  useEffect(() => {
    ...
    return () => { observer?.disconnect(); window.cancelAnimationFrame(frame); media.removeEventListener('change', setup); };
  }, []);
  ...
  return <section id="sobre" ref={root} className={styles.root} aria-labelledby="author-title">
    <header className={styles.topline} data-author-motion="fade">
      <p>02 / POR TRÁS DO CUIDADO</p><span aria-hidden="true" /><p>A BELEZA TAMBÉM ESTÁ NAS PESSOAS.</p>
    </header>
    ...
    <div className={styles.stage}>
      ...
      <figure className={styles.photo} data-author-motion="photo">
        <Image src={site.images.about} alt="Dra. Laís Paula em um registro de sua identidade profissional" fill sizes="(max-width: 700px) 51vw, (max-width: 1100px) 56vw, 54vw" />
      </figure>
      ...
      <aside className={styles.sideNote} aria-label="Mais que procedimentos, identidade."><span>MAIS QUE PROCEDIMENTOS, IDENTIDADE.</span><i aria-hidden="true" /><b aria-hidden="true" /></aside>
      ...
      <svg className={`${styles.thread} ${styles.threadDesktop}`} viewBox="0 0 1600 110" preserveAspectRatio="none" aria-hidden="true" focusable="false">
        <defs><radialGradient id={`${uid}-author-pearl`} cx="30%" cy="25%"><stop stopColor="#fff9ee" /><stop offset=".5" stopColor="#d6b47a" /><stop offset="1" stopColor="#9a7045" /></radialGradient></defs>
      ...
      <svg className={`${styles.thread} ${styles.threadMobile}`} viewBox="0 0 430 546" preserveAspectRatio="none" aria-hidden="true" focusable="false">
        <defs><radialGradient id={`${uid}-author-pearl-mobile`} cx="30%" cy="25%"><stop stopColor="#fff9ee" /><stop offset=".5" stopColor="#d6b47a" /><stop offset="1" stopColor="#9a7045" /></radialGradient></defs>
    ...
    <footer className={styles.closing}>
      ...
      <div className={styles.closingRow}>
        <div className={styles.closingMonogram} aria-hidden="true"><strong>lp.</strong><span /></div>
        <p className={styles.closingStatement}><span data-author-motion="final">Precisão em cada escolha.</span><em data-author-motion="final">Cuidado em cada encontro.</em></p>
        <div className={styles.closingSeal}><span aria-hidden="true" /><BrandSeal id={uid} /></div>
      </div>
      <p className={styles.closingNote} data-author-motion="final">Equilíbrio, presença e intenção em cada detalhe.</p>
    </footer>
  </section>;
}
